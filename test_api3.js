...
async function main() {
  ...
  const nbId = data.notebook.id;
  console.log("Notebook:", nbId);
  
  console.log("Adding text source...");
  res = await fetch(`${url}/v1/notebooks/${nbId}/sources/text`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: "Source 1", content: "Dies ist ein Testtext über Künstliche Intelligenz. KI ist sehr cool und nützlich." }) });
